...
const SubscriptionPage = ({ isAr, setPage, userProfile, setUserProfile, onSaveProfile, showToast }) => {
  ...
  const handleUpgradeToBro = () => {
    ...
    showToast(isAr ? 'Upgraded to Bro Plan! +100 Credits 🚀' : 'Upgraded to Bro Plan! +100 Credits 🚀');
  };
    ...
    <div className="min-h-screen pt-32 px-6 pb-20 max-w-[1200px] mx-auto text-theme-primary">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch mb-12">
        ...
        <div className="bg-[var(--accent-lilac)] border-4 border-black p-8 rounded-[2.5rem] shadow-brutal-lg flex flex-col justify-between relative text-black transform lg:-translate-y-4 z-10">
          ...
          >
            {isAr ? 'Upgrade for $5/mo 🚀' : 'Upgrade for $5/mo 🚀'}
          </button>
        </div>
...
const Dashboard = ({ isAr, type, setPage, mouseX, mouseY, userProfile, onTriggerTool }) => {
  ...
  const studentFeatures = [
    ...
    { id: 'tool_university_directory', num: '06', titleAr: 'دليل الجامعات بالذكاء الاصطناعي', titleEn: 'AI University Directory', descAr: 'دليل الجامعات السعودية والبحث العالمي بالذكاء الاصطناعي.', descEn: 'KSA Universities & Global AI Search.', points: 10, color: 'bg-[var(--accent-peach)]', span: 'span-2', icon: <Globe className="bento-bg-icon" /> },
  ...
  const proFeatures = [
    ...
    { id: 'tool_salary', num: '02', titleAr: 'منحنى الرواتب', titleEn: 'Salary Curve', descAr: 'تصور دقيق لمقاييس نمو الرواتب بمرور الوقت.', descEn: 'Visualize precise salary scaling metrics.', points: 15, color: 'bg-[var(--accent-mint)]', span: 'span-2', icon: <TrendingUp className="bento-bg-icon" /> },
    { id: 'tool_important_courses', num: '03', titleAr: 'Upskilling Certifications', titleEn: 'Upskilling Certifications', descAr: 'تحديد الشهادات الاحترافية للوصول إلى مستويات أعلى.', descEn: 'Identify certifications to secure higher tiers.', points: 5, color: 'bg-[var(--accent-yellow)]', icon: <Award className="bento-bg-icon" /> },
    { id: 'tool_career_pivot', num: '04', titleAr: 'Career Pivot Roadmap', titleEn: 'Career Pivot Roadmap', descAr: 'وضع خارطة طريق خطوة بخطوة للانتقال المهني.', descEn: 'Establish step-by-step transition roadmaps.', points: 15, color: 'bg-[var(--accent-lilac)]', span: 'span-2', icon: <Compass className="bento-bg-icon" /> },
    { id: 'tool_ai_jobs_salary', num: '05', titleAr: 'البحث المباشر عن الرواتب', titleEn: 'Live Salary Search', descAr: 'يستخرج الرواتب الحية باستخدام البحث المباشر عبر الويب.', descEn: 'Scrapes live salaries using grounded Web Search.', points: 15, color: 'bg-[var(--accent-coral)]', span: 'span-2', icon: <Search className="bento-bg-icon" /> },
...
const ToolCurriculumPath = createAIToolComponent({
  titleAr: 'مخطط المسار التعليمي المستقبلي', titleEn: 'Future Educational Path Blueprint',
  useProfilePersona: true, hasInput2: false,
  ph1Ar: 'أدخل التخصص المستهدف (مثل: هندسة البرمجيات)', ph1En: 'Enter target major (e.g. Software Engineering)',
  btnAr: 'توليد المخطط', btnEn: 'Generate Blueprint',
...
const ToolJobTitles = createAIToolComponent({
  titleAr: 'المسميات الوظيفية وخريطة سوق العمل', titleEn: 'Career Titles Mapping',
  useProfilePersona: true, hasInput2: false,
  ph1Ar: 'أدخل التخصص أو المجال', ph1En: 'Enter major or industry domain',
  btnAr: 'استخراج المسميات', btnEn: 'Generate Job Titles',
...
const ToolImportantCourses = createAIToolComponent({
  titleAr: 'تطوير المهارات والشهادات الأساسية', titleEn: 'Upskilling & Core Certifications',
  useProfilePersona: true, hasInput2: false,
  ph1Ar: 'أدخل تخصصك أو مجالك المستهدف', ph1En: 'Enter your major or target domain',
  btnAr: 'استكشاف الشهادات', btnEn: 'Unlock Industry Certifications',
...
const ToolCareerPivot = createAIToolComponent({
  titleAr: 'خطة الانتقال المهني', titleEn: 'Career Transition Blueprint',
  useProfilePersona: false, hasInput2: true,
  ph1Ar: 'دورك الحالي أو مجالك (مثل: دعم العملاء)', ph1En: 'Your current role or domain (e.g. Customer Support)',
  ph2Ar: 'الدور أو المجال المستهدف (مثل: محلل بيانات)', ph2En: 'Your target role or domain (e.g. Data Analyst)',
  btnAr: 'توليد خطة الانتقال', btnEn: 'Generate Transition Roadmap',
...
const ToolDefineRoadmap = createAIToolComponent({
  titleAr: 'تحديد معالم الترقية', titleEn: 'Plot Promotional Milestones',
  useProfilePersona: false, hasInput2: false,
  ph1Ar: 'ما هو هدفك المهني النهائي؟ (مثل: الرئيس التنفيذي للتكنولوجيا)', ph1En: 'What is your ultimate career goal? (e.g. Chief Technology Officer)',
  btnAr: 'تحديد المعالم', btnEn: 'Generate Milestones',
...
const ToolGraduationIdeas = createAIToolComponent({
  titleAr: 'أفكار مشاريع تخرج قابلة للتسويق', titleEn: 'Marketable Capstone Project Ideas',
  useProfilePersona: true, hasInput2: false,
  ph1Ar: 'أدخل تخصصك الدراسي', ph1En: 'Enter your study major',
  btnAr: 'اقتراح مشاريع', btnEn: 'Generate Project Proposals',
...
const ToolJobHunt = createAIToolComponent({
  titleAr: 'الوظائف الشاغرة والفرص المتاحة', titleEn: 'Open Vacancies & Corporate Placements',
  useProfilePersona: true, hasInput2: true,
  ph1Ar: 'التخصص أو المسمى المستهدف', ph1En: 'Target Major or Title',
  ph2Ar: 'مهاراتك الأساسية', ph2En: 'Your key skills',
  btnAr: 'استكشاف الفرص', btnEn: 'Unlock Opportunities',
...
const ToolCareerTest = ({ isAr, setPage, userContext, userProfile, setUserProfile, onSaveProfile, showToast, onDeductPoints }) => {
  ...
  const handleAIPerspective = async () => {
    ...
    try {
      ...
      console.error(e);
      setAiPerspective(isAr ? 'تعذر جلب التحليل في الوقت الحالي. حاول مجدداً.' : 'Failed to fetch analysis at this time. Try again.');
    } finally {
      setAiLoading(false);
    }
  };
            ...
            {aiReport && (
              <div className="page-enter text-black space-y-6">
                <div className="p-4 bg-[var(--accent-mint)] border-2 border-black rounded-2xl font-black flex items-center gap-2 text-sm justify-between shadow-[2px_2px_0_#000]">
                  <span>🚀 {isAr ? 'Report validated with live web data' : 'Report validated with live web data'}</span>
...
export default function App() {
  ...
  const triggerToolWithCredits = (targetTool, pointsCost) => {
    ...
    if (!isBro && pointsCost > 0) {
      ...
      showToast(lang === 'ar' ? `تم خصم ${pointsCost} نقطة لبدء الأداة.` : `Deducted ${pointsCost} credits to run diagnostic.`);
    }
  ...
  const handleSetPage = (newPage) => {
    ...
    if (!userProfile.isLoggedIn && !bypassPages.includes(newPage)) {
      showToast(lang === 'ar' ? 'يجب تسجيل حسابك الموثق أولاً للوصول إلى أدوات الذكاء الاصطناعي!' : 'Please sign up or log in first to access premium modules!');
      handleTransition(() => {
        setPage('auth_signup');
        setIsMenuOpen(false);
      });
