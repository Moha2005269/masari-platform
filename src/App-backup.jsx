          ...
          <Magnetic strength={0.1} className="w-full">
            <button type="button" onClick={async () => {
              try {
                ...
                showToast(isAr ? 'فشل تسجيل الدخول باستخدام جوجل: ' + err.message : 'Google sign-in failed: ' + err.message);
              }
...
const ToolCareerTest = ({ isAr, setPage, userContext, userProfile, setUserProfile, onSaveProfile, showToast, onDeductPoints }) => {
  ...
  const handleAIPerspective = async () => {
    ...
    try {
      ...
      console.error(e);
      setAiPerspective(isAr ? 'Failed to fetch analysis at this time. Try again.' : 'Failed to fetch analysis at this time. Try again.');
    } finally {
      setAiLoading(false);
    }
  };
...
export default function App() {
  ...
  const triggerToolWithCredits = (targetTool, pointsCost) => {
    ...
    if (!isBro && pointsCost > 0) {
      ...
      showToast(lang === 'ar' ? `تم خصم ${pointsCost} نقطة لتشغيل الأداة.` : `Deducted ${pointsCost} credits to run diagnostic.`);
    }
  ...
  const handleSetPage = (newPage) => {
    ...
    if (!userProfile.isLoggedIn && !bypassPages.includes(newPage)) {
      showToast(lang === 'ar' ? 'يجب تسجيل حساب أو تسجيل الدخول أولاً للوصول إلى أدوات الذكاء الاصطناعي!' : 'Please sign up or log in first to access premium modules!');
      handleTransition(() => {
        setPage('auth_signup');
        setIsMenuOpen(false);
      });
